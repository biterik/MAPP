...
/*--------------------------------------------
 ...
 --------------------------------------------*/
ForceField_DMD_I::
ForceField_DMD_I(MAPP* mapp) : ForceField(mapp)
{
    no_i=0;
    allocated=0;
    eam_mode=NOT_SET;
...
/*--------------------------------------------
 destructor
 --------------------------------------------*/
ForceField_DMD_I::~ForceField_DMD_I()
{
...
/*--------------------------------------------
 energy calculation
 --------------------------------------------*/
TYPE0 ForceField_DMD_I::energy_calc()
{
    TYPE0* x;
    atoms->vectors[x_n].ret(x);
    TYPE0* f;
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceField_DMD_I::coef(int narg,char** arg)
{
    TYPE0 kb,T,hbar;
    if (narg<8)
    ...
    {
        eam_mode=SET_FL;
        set_setfl(narg-7,&arg[7]);
    }
    else if(strcmp(arg[6],"FuncFL")==0)
    {
        eam_mode=FUNC_FL;
        set_funcfl(narg-7,&arg[7]);
    }
...
/*--------------------------------------------
 read setfiles
 --------------------------------------------*/
void ForceField_DMD_I::set_setfl(int no_files
                               ,char** file_names)
{
...
/*--------------------------------------------
 read setfiles
 --------------------------------------------*/
void ForceField_DMD_I::set_funcfl(int no_files
                                ,char** file_names)
{
    if(no_files!=1)
...
/*--------------------------------------------
 read setfiles
 --------------------------------------------*/
void ForceField_DMD_I::set_fs(int no_files
                            ,char** file_names)
{
    if(no_files!=1)
        error->abort("one file is needed for eam");
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
int ForceField_DMD_I::line_read(FILE* file
                              ,char*& line)
{
    int lenght;
    int eof=0;
...
/*--------------------------------------------
 clean up the arrays
 --------------------------------------------*/
void ForceField_DMD_I::clean_up()
{
    if(allocated==0)
        return;
...
/*--------------------------------------------
 allocate the arrays
 --------------------------------------------*/
void ForceField_DMD_I::allocate()
{
...
/*--------------------------------------------
 set the arrays
 --------------------------------------------*/
void ForceField_DMD_I::set_arrays()
{
    if(eam_mode==FUNC_FL || eam_mode==SET_FL)
    {
        for(int i=0;i<no_types;i++)
...
/*--------------------------------------------
 allocate the arrays
 --------------------------------------------*/
void ForceField_DMD_I::interpolate(int n,TYPE0 delta
                                 ,TYPE0** spline)
{
...
/*--------------------------------------------
 allocate the arrays
 --------------------------------------------*/
void ForceField_DMD_I::interpolate_m(int n,TYPE0 delta
                                   ,TYPE0** spline)
{
    spline[0][1]=spline[1][0]-spline[0][0];
    spline[1][1]=0.5*(spline[2][0]-spline[0][0]);
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceField_DMD_I::set_weight_abs(int n)
{
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceField_DMD_I::create_2nd_neigh_lst()
{
...
/*--------------------------------------------
 return M_{ij}^{\alpha}
 --------------------------------------------*/
TYPE0 ForceField_DMD_I::calc_ent(TYPE0 x)
{
...
/*--------------------------------------------
 claculate F and dF and dFF
 --------------------------------------------*/
void ForceField_DMD_I::c_d_calc()
{
    TYPE0* x;
    atoms->vectors[x_n].ret(x);
    TYPE0* E;
    ...
    for(iatm=0;iatm<natms;iatm++)
    {
        ...
        for(int j=0;j<neigh_lst_sz[iatm];j++)
        {
            ...
            for(int itype=0;itype<no_types;itype++)
            {
                //cout << 0.5*(n[icomp+itype]-n[jcomp+itype])+Mat(crd[iatm],crd[jatm],itype) <<endl;
                tmp0=exp(0.5*(n[icomp+itype]-n[jcomp+itype])+Mat(crd[iatm],crd[jatm],itype));
                tmp1=exp(0.5*(n[jcomp+itype]-n[icomp+itype])+Mat(crd[iatm],crd[jatm],itype));
...
/*--------------------------------------------
 claculate F and dF and dFF
 --------------------------------------------*/
TYPE0 ForceField_DMD_I::calc_g(int chk,TYPE0 alpha,
                             TYPE0* a,TYPE0* g)
{
    TYPE0* x;
    atoms->vectors[x_n].ret(x);
    ...
    TYPE0 p,p2,p3,p4,tmp0,tmp1,s_ij,t_ij;
    int m;
    ...
    for(iatm=0;iatm<natms;iatm++)
    {
        ...
        for(int j=0;j<neigh_lst_sz[iatm];j++)
        {
            ...
            for(int itype=0;itype<no_types;itype++)
            {
                tmp0=exp(0.5*(n[icomp+itype]-n[jcomp+itype])+Mat(crd[iatm],crd[jatm],itype));
                tmp1=exp(0.5*(n[jcomp+itype]-n[icomp+itype])+Mat(crd[iatm],crd[jatm],itype));
                tmp0*=(s[icomp+itype]-s[jcomp+itype]);
                tmp1*=(s[icomp+itype]-s[jcomp+itype]);
                
                t_ij=0.5*(c[icomp+itype]*(1.0-c[jcomp+itype])*tmp0+c[jcomp+itype]*(1.0-c[icomp+itype])*tmp1);
